...
class RepoCopyTemplateV1
  ...
  def execute
    ...
    begin
      tempfile = Tempfile.new('kinetic_git_key')
      tempfile.write(@info_values["ssh_key"])
      tempfile.rewind

      path = Dir.mktmpdir("kinetic_git_handler")
      puts path

      sshSessionFactory = JgitHelperClasses::JshSessionFactory.new(tempfile.path.to_java)
      ...
      clone.setURI(@parameters["source_repository"])
        ...
        .setTransportConfigCallback(
          JgitHelperClasses::TransportCallback.new(sshSessionFactory)
        )
        .call()
